        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="flex m-2 p-2">
                <a href="{{ route('admin.reservations.index') }}"
                    class="px-4 py-2 bg-green-500 hover:bg-green-700 rounded-lg text-white">Reservation Index</a>
            </div>
            <div class="m-2 p-2 bg-slate-100 rounded">
                <div class="space-y-8 divide-y divide-gray-200 w-1/2 mt-10">
                    <form method="POST" action="{{ route('admin.reservations.store') }}">
                        ...
                        <div class="sm:col-span-6">
                                    <label for="addon" class="block text-sm font-medium text-gray-700"> &nbsp;Add-ons
                                    </label>
                                    <div class="mt-1">
                                        <input type="text" id="addon" name="addon"
                                            value="{{ $reservation->addon ?? '' }}"
                                            class="block w-full appearance-none bg-white border border-gray-400 rounded-md py-2 px-3 text-base leading-normal transition duration-150 ease-in-out sm:text-sm sm:leading-5" />
                                    </div>
                                    <p style="font-size:12px;color:gray">&nbsp;(Note: Add-ons are Refrigerator(₱500) & Fans(₱300))</p>
                                    @error('addon')
                                        <div class="text-sm text-red-400">{{ $message }}</div>
                                    @enderror
                                </div>
